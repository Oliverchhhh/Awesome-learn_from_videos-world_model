---
name: 添加论文 Add Paper
about: 建议添加一篇新论文 Suggest adding a new paper
title: '[Paper] 论文标题'
labels: 'paper-suggestion'
assignees: ''
---

## 论文信息 Paper Information

**标题 Title:**


**作者 Authors:**


**机构 Institution:**


**发表时间 Publication Date:**
YYYY-MM

**论文链接 Paper Link:**
- arXiv: 
- 或会议/期刊链接: 

**代码链接 Code Link (如果有):**


**项目主页 Project Page (如果有):**


**演示视频 Demo Video (如果有):**


## 分类 Category

请选择论文所属分类：
- [ ] 🎬 从视频学习 (Learn from Videos)
- [ ] 🌍 世界模型 (World Model)

## 论文简介 Brief Description

请用1-2句话描述论文的核心贡献和创新点：




## 额外信息 Additional Information

- 为什么推荐这篇论文？
- 这篇论文有什么特别之处？
- 其他想要补充的信息




## 参考格式 Format Reference

```markdown
- **论文标题**, 会议/期刊 年份. [[paper]](链接) [[code]](链接) [[website]](链接)
```

---

感谢您的贡献！我们会尽快审核并添加。
Thank you for your contribution! We will review and add it soon.

